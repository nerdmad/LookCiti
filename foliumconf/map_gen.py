...
def generate_map(latitude, longitude) -> None:
    if latitude == None or longitude == None:
        latitude, longitude = 55.75, 37.62
    ...
    folium.Marker(
        ...
        popup = "Ваша локация",
        icon = folium.Icon(color='lightred', icon='info-sign') #for some reason i can't change the icon itself???
    ).add_to(m)
...
#this function is horrible with those if-else constructions, please don't kill me
def find_closest(latitude, longitude, option):
    global ans, diff_df
    diff_df = {
        'distance': [],
        'name': []
    ...
    for j in range(0, len(data)):
        lat_dif = abs(latitude - float(data.iloc[j]['lat']))
        lon_dif = abs(longitude - float(data.iloc[j]['lon']))
        dif = round(sqrt(((lat_dif*111000)**2 + (lon_dif*111000)**2)))
        if dif <= 2000:
            diff_df['distance'].append(dif)
            diff_df['name'].append(data.iloc[j]['name'])
    # giving answer to the user
    if option == 'ans':
        if diff_df['name'] == []:
            ans = 'Не найдено ни одной близкой к Вам достопримечательности. Может быть, Вы забыли вырубить VPN?' #unused so far, but imma keep it for future if i'll need it
        else:
            ans = f'Список ближайших достопримечательностей:\n'
            for k in range(0, len(diff_df['name'])):
                if k == len(diff_df['name'])-1:
                    ans += f'{k+1}. {diff_df['name'][k]}, расстояние: {diff_df['distance'][k]} метров.\n\n'
                    ans += f'Пожалуйста, выберите локацию, к которой будете идти:'
                else:
                    ans += f'{k+1}. {diff_df['name'][k]}, расстояние: {diff_df['distance'][k]} метров;\n'
        return ans
    #providing information for user buttons
    elif option == 'list':
        name_list = diff_df['name']
        return name_list

    
#print(find_closest(None, None, 'list'/'ans')) for testing