...
def find_closest(latitude, longitude):
    ...
    if diff_df['name'] == []:
        ans = 'Не найдено ни одной близкой к Вам достопримечательности. Может быть, Вы забыли вырубить VPN?'
    else:
        for k in range(0, len(diff_df['name'])):
            if k == len(diff_df['name'])-1:
                ans += f'{diff_df['name'][k]}, расстояние: {diff_df['distance'][k]} метров.'
            else:
                ans += f'{diff_df['name'][k]}, расстояние: {diff_df['distance'][k]} метров;\n'
...
#print(find_closest(None, None)) for testing